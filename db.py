...
def save_career_analysis(
    ...
    # Remove any legacy seed candidates
    seed_ids = {"career_rajkishore_default", "career_alex_chen", "career_devin_vance"}
    local_records = [r for r in local_records if r.get("analysis_id") not in seed_ids]
...
def get_user_career_analyses(user_uid: str) -> List[Dict[str, Any]]:
    ...
    # Filter out legacy seed profiles completely
    seed_ids = {"career_rajkishore_default", "career_alex_chen", "career_devin_vance"}
    filtered_results = [r for r in results if r.get("analysis_id") not in seed_ids]
